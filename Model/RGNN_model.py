...
def scatter_add(src, index, dim, dim_size):
    ...
    ----------
    src : torch.Tensor
        Source tensor containing values to add.
    index : torch.Tensor
        Indices tensor specifying locations to add values.
    dim : int
        The dimension along which to index.
    dim_size : int
        The size of the output tensor along the specified dimension.

    Returns:
    -------
    output : torch.Tensor
        The resulting tensor after scatter-add operation.
    """
    # Ensure the index tensor has the same size as the src along the specified dimension
    index = index.unsqueeze(-1).expand_as(src) if src.dim() > 1 else index

    # Initialize the output tensor with zeros
    output_shape = list(src.size())
    output_shape[dim] = dim_size
    output = torch.zeros(*output_shape, dtype=src.dtype, device=src.device)

    # Scatter-add values from src into output along the specified dimension
