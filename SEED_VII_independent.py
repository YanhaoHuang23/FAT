...
def main_LOSO_all_sessions():
    ...
    for session in sessions:
        print(f"\nProcessing {session}...")
        dir = f'/mnt/DATA-2/SEED-VII/DE/{session}/'  # 根据当前 session 设置数据路径
        ...
        for sub_i in range(sub_num):
            print(f"Subject {sub_i + 1}/{sub_num}: {file_list[sub_i]}")
            load_path = os.path.join(dir, file_list[sub_i])
            data_test, label_test = load_DE_SEED(load_path)

            # 对测试集进行 z-score 标准化
            data_test = data_test.reshape(data_test.shape[0], -1)
            data_test = zscore(data_test, axis=0)
            data_test = data_test.reshape(-1, 62, 5)  # 请根据实际的通道数和频带数调整
            ...
            for train_i in range(train_num):
                train_path = os.path.join(dir, train_list[train_i])
                data_train_i, label_train_i = load_DE_SEED(train_path)

                # 对训练集进行 z-score 标准化
                data_train_i = data_train_i.reshape(data_train_i.shape[0], -1)
                data_train_i = zscore(data_train_i, axis=0)
                data_train_i = data_train_i.reshape(-1, 62, 5)

                data_train_list.append(data_train_i)
                label_train_list.append(label_train_i)
            ...
            model = FATGraph(n_classes=7, num_channels=num_channels, num_freq_bands=num_freq_bands, use_dynamic_graph=True).to(device)
        ...
        # 输出当前 session 的结果
        print(f"\nResults for {session}:")
        print('>>> LOSO test acc (per subject): ', acc_all)
        print(f'>>> {session} LOSO test mean acc: {acc_mean:.4f}')
        print(f'>>> {session} LOSO test std acc: {acc_std:.4f}')
    ...
    for session in sessions:
        print(f"\n{session} Results:")
        print('>>> LOSO test acc (per subject): ', session_results[session]['acc_all'])
        print(f'>>> {session} LOSO test mean acc: {session_results[session]["mean"]:.4f}')
        print(f'>>> {session} LOSO test std acc: {session_results[session]["std"]:.4f}')
    # 输出总体结果
    print('\nOverall results across all sessions:')
    print('>>> Overall LOSO test acc (all subjects, all sessions): ', acc_all_sessions)
    print(f'>>> Overall LOSO test mean acc: {overall_acc_mean:.4f}')
    print(f'>>> Overall LOSO test std acc: {overall_acc_std:.4f}')
